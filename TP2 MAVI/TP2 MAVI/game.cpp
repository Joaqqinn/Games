...
void game::initializer_menu() {
	...
	texto_score->setPosition(350, 650);
	...
	texto_points->setPosition(1150, 650);
...
void game::drawing() {
	...
	for (int i = 0; i < 15; i++) { //DIBUJAMOS ASTEROIDES GRANDES
		if (big_asteroid[i]) {
			ventana1->draw(big_asteroid[i]->get_spr_big());
		}
	}
	for (int i = 0; i < 15; i++) { //DIBUJAMOS ASTEROIDES PEQUEÑOS
		if (small_asteroids[i]) {
	...
	if (_gameover) { // SI GAMEOVER EL VERDADERO DIBUJAMOS
		ventana1->draw(*texto_repeat);
	...
	if (explosion_asteroid) { //SI BALA Y ASTEROIDES COLISIONAN DIBUJAMOS ANIMACION
